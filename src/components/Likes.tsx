...
const Likes = ({item}: {item: MediaItemWithOwner}) => {
  ...
  const getLikeCount = async () => {
    try {
      const countResponse = await getCountByMediaId(Number(item.media_id)); // Convert item.media_id to a number
  ...
  const handleLike = async () => {
    try {
      ...
      if (likeState.userLike) {
        ...
        await postLike(Number(item.media_id), token); // Convert item.media_id to a number
        getLikes();
        getLikeCount();
      }
    } catch (e) {
